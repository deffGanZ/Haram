...
require("./Gans")(haikal, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
if (anu.action == 'add') {
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Welcome Kak *@${num.split("@")[0]}* Selamat Datang Di Group *${metadata.subject}* `})
} else if (anu.action == 'remove') {
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Selamat Tinggal *@${num.split("@")[0]}* Semoga Kamu Tenang Yah Disana`})
} else if (anu.action == 'promote') {
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Telah di Promosikan Menjadi Admin Di group ${metadata.subject}`  })
} else if (anu.action == 'demote') {
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Telah Diturunkan Menjadi Anggota Di Group ${metadata.subject}`})
  }
}
} catch (err) {
console.log(err)
...
connectToWhatsApp();
  ...
  haikal.sendMessage('6283873494075' + "@s.whatsapp.net", { text: `*Sukses Terhubung Dengan Baik*\n\nSilahkan Gunakan BOT Dengan Bijak` });
}
